      ...
      <ResponsiveContainer width="100%" height={180}>
        <PieChart>
          <Pie data={data} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={60} label>
            {data.map((entry, idx) => (
              <Cell key={`cell-${idx}`} fill={BLUE_COLORS[idx % BLUE_COLORS.length]} />
            ))}
          </Pie>
          <Legend />
          <Tooltip contentStyle={{ background: '#f3e8ff', borderColor: '#2563eb' }} />
  ...
  const headerActions = (
    <>
      ...
      <div className="flex items-center gap-2 bg-gradient-to-r from-blue-100 to-blue-200 px-3 py-2 rounded-lg shadow">
        <span className="w-8 h-8 bg-gradient-to-br from-blue-400 to-blue-600 rounded-full flex items-center justify-center font-bold text-white shadow">A</span>
