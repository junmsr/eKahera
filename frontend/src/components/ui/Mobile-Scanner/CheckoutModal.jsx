    ...
    <Modal isOpen={isOpen} onClose={onClose} title="">
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <label className="flex items-center gap-3">
            <input type="radio" name="payment" checked={method === 'GCash'} onChange={() => setMethod('GCash')} />
            <span className="text-blue-900">GCash</span>
          </label>
          <label className="flex items-center gap-3">
            <input type="radio" name="payment" checked={method === 'PayMaya'} onChange={() => setMethod('PayMaya')} />
        ...
        <div className="flex items-center justify-end">
          <Button label="Pay" variant="primary" onClick={() => onPay(method || 'Cash')} microinteraction />
        </div>
      </div>
    </Modal>
  );
